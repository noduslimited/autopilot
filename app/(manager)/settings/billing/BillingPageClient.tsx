    ...
    <div className="rounded-card border border-border-default bg-card-bg py-4 px-5">
      ...
      {needsPlan ? (
        <div className="mt-5 border-t border-border-default pt-4">
          <h2 className="mb-3 text-section-heading text-text-primary">Choose a plan</h2>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {TIER_ORDER.map((tier) => {
              const display = TIER_DISPLAY[tier];
              return (
                <div key={tier} className="flex flex-col rounded-card border border-border-default p-4">
